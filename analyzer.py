...
def makeGithubRequest(url, params=None):
    headers = {
...
@st.cache_data(ttl=3600)
def getRepoStargazers(owner, repo):
    star_data = []
    page = 1
    per_page = 100
    ...
    with st.spinner(f"Fetching star history for {owner}/{repo}..."):
        while True:
            ...
            response = makeGithubRequest(base_url, params=params)
            if response is None:
                return None
...
def getRepoLanguages(owner, repo):
    ...
    response = makeGithubRequest(url)
...
def getUserRepos(owner):
    ...
    while True:
        ...
        response = makeGithubRequest(url, params=params)
        if response is None:
            break
        current_page_data = response.json()
...
@st.cache_data(ttl=3600)
def getUserEvents(owner):
    url = f"https://api.github.com/users/{owner}/events/public"
    response = makeGithubRequest(url, params={"per_page": 100})
    return response.json() if response else None


def buildStarDF(raw_star_data):
...
def analyzeUserData(owner, profile, user_repos, events):
    total_open_issues = 0
    total_watchers = 0
    lang_counts = defaultdict(int)
    repo_star_counts = defaultdict(int)
    most_popular = {'name': 'N/A', 'stars': 0, 'forks': 0}
    active_repos = 0
    ...
    for repo in user_repos:
        ...
        if stars > most_popular['stars']:
            most_popular.update({'name': repo['name'], 'stars': stars, 'forks': forks})
    ...
    top_repo_name = most_popular['name']

    if top_repo_name != 'N/A':
        full_top_repo = next((r for r in user_repos if r['name'] == top_repo_name), None)
        if full_top_repo:
            top_repo_info['topics'] = full_top_repo.get('topics', [])
            top_repo_info['has_pages'] = full_top_repo.get('has_pages', False)
            top_repo_info['has_wiki'] = full_top_repo.get('has_wiki', False)
            ...
            top_repo_info['oldest_open_issue_lifespan_days'] = oldest_open_issue_days

        detailed_languages = getRepoLanguages(owner, top_repo_name)
        repo_stats = getRepoStats(owner, top_repo_name)
        ...
        if repo_stats and repo_stats['code_frequency']:
            ...
            for week in repo_stats['code_frequency']:
                ...
                data.append({'Week': week_start, 'Additions': week[1], 'Deletions': abs(week[2]), 'Net Change': week[1] + week[2]})
            code_df = pd.DataFrame(data)

    user_orgs = getUserOrgs(owner)

    recent_activity_score = 0
    event_counts = defaultdict(int)
    external_contribs = defaultdict(lambda: defaultdict(int))
        ...
        if event_type in ('PushEvent', 'PullRequestEvent'):
            try:
                ...
                if repo_owner.lower() != owner.lower():
                    external_contribs[repo_owner][event_type] += 1
    ...
    report = {
        'user_id': profile.get('login'),
        'name': profile.get('name', 'N/A'),
        'hireable': profile.get('hireable', False),
        'location': profile.get('location', 'N/A'),
        'member_since': profile.get('created_at', 'N/A'),
        'followers': profile.get('followers', 0),
        'public_repos_count': profile.get('public_repos', 0),
        'public_gists_count': profile.get('public_gists', 0),
        'total_stars_owned': total_stars,
        'total_forks_owned': total_forks,
        ...
        'user_orgs': user_orgs,
        'top_language': max(lang_counts.items(), key=operator.itemgetter(1))[0] if lang_counts else 'N/A',
        'repos_with_recent_activity': active_repos,
        'recent_activity_score': recent_activity_score,
        'recent_event_types': dict(event_counts),
        'top_external_projects': sorted(external_contribs.items(), key=lambda item: sum(item[1].values()), reverse=True)[:5],
...
def createHtmlReport(report):
    ...
    html_content += f"<p><strong>Activity Level:</strong> {activityBadge(report['recent_activity_score'])}</p>"
    ...
    if report['top_external_projects']:
        html_content += "<ul>"
        for ownerItem, activity in report['top_external_projects']:
            total = sum(activity.values())
            pushes = activity.get('PushEvent', 0)
            prs = activity.get('PullRequestEvent', 0)
            html_content += f"<li>**{ownerItem}** (Total: {total} events | Pushes: {pushes} | PRs/Issues: {prs})</li>"
        html_content += "</ul>"
    else:
...
def showRepoStars(owner, repo, df):
    ...
    fig = px.line(df, x='starred_at', y='cumulative_stars', title=f"Cumulative Stars for {owner}/{repo}", labels={'starred_at': 'Date Starred', 'cumulative_stars': 'Total Stars'}, markers=True)
    fig.update_traces(line=dict(color='#ff6347'))
    st.plotly_chart(fig, use_container_width=True)


def showUserReport(report):
    ...
    with col2:
        ...
        m3.metric("Repos with Recent Activity", report['repos_with_recent_activity'], help="Repositories with a push event in the last 12 months.")
        m4, m5 = st.columns(2)
        ...
        st.markdown(f"""
        ### Activity Summary
        - **Activity Level (Based on Recent Events):** {activityBadge(report['recent_activity_score'])}
    ...
    with c2:
        ...
        if not star_df.empty:
            star_fig = px.bar(star_df, x='Stars', y='Repository', orientation='h', title='Top 10 Starred Repos', color_discrete_sequence=['#1f77b4'])
    ...
    if report['top_repo_data']:
        ...
        """)
        m_col2.metric("Est. Oldest Issue Lifespan (Days)", report['top_repo_data'].get('oldest_open_issue_lifespan_days', 'N/A'), help="Max days an open issue/PR could have existed, based on repo creation.")
    ...
    with col_code_1:
        ...
        if detailed_langs and sum(detailed_langs.values()) > 0:
            ...
            lang_data = [{'Language': lang, 'Bytes': bytes} for lang, bytes in detailed_langs.items()]
    ...
    if not commit_df.empty:
        commit_fig = px.bar(commit_df, x='Week', y='Total Commits', title='Total Commits by Week', color_discrete_sequence=['#ff6347'])
    ...
    if not code_df.empty:
        ...
        fig = px.line(code_df, x='Week', y=['Additions', 'Deletions'], title='Weekly Lines of Code Added vs. Deleted', labels={'value': 'Lines of Code', 'variable': 'Action'})
        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    if not push_df.empty:
        ...
        time_fig = px.bar(push_df, x='Hour', y='Pushes', title='Push Events by Hour of Day (UTC)', color_discrete_sequence=['#2ca02c'])
        st.plotly_chart(time_fig, use_container_width=True)

    st.markdown("---")
    st.subheader("⬇️ Export Analysis")

    html_output = createHtmlReport(report)

    st.download_button(label="Download Full Analysis (Auto-PDF HTML)", data=html_output, file_name=f"{report['user_id']}_github_analysis_{datetime.now().strftime('%Y%m%d')}.html", mime="text/html", help="Downloads a self-contained HTML file. Open this file and your browser will immediately prompt you to save it as a PDF.")
...
def mainApp():
    ...
    user_input = st.text_input("GitHub URL or Username", placeholder="Enter URL or Username here...", key="github_input")
    if not user_input:
        st.stop()

    repoOwner, repoName = parseGitHubUrl(user_input)
    if not repoOwner:
    ...
    if repoName:
        st.info(f"Analyzing Repository: **{repoOwner}/{repoName}**")
        raw_star_data = getRepoStargazers(repoOwner, repoName)
        if raw_star_data is not None:
            processed_star_data = buildStarDF(raw_star_data)
            showRepoStars(repoOwner, repoName, processed_star_data)
        else:
            st.error(f"🛑 Could not retrieve data for {repoOwner}/{repoName}. Check URL and rate limits.")
    else:
        st.info(f"Analyzing User Profile: **{repoOwner}**")
        with st.spinner("Fetching user profile, repositories, organizations, and events..."):
            profile_data = getUserProfile(repoOwner)
            user_repos = getUserRepos(repoOwner)
            events_data = getUserEvents(repoOwner)

        if profile_data and user_repos is not None:
            user_report = analyzeUserData(repoOwner, profile_data, user_repos, events_data)
            showUserReport(user_report)
        else:
            st.error(f"🛑 Failed to fetch full data for user **{repoOwner}**. Check the username or if the user exists.")
